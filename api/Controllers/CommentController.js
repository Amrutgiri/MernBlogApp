...
export const editComment = async (req, res, next) => {
  try {
    ...
    const editCommnet =await Comment.findByIdAndUpdate(req.params.commentId,{
    ...
    res.status(200).json(editCommnet);
...
export const getcomments = async (req, res, next) => {
  if(!req.user.isAdmin){
    return next(errorHandler(403,"You are not authorized to access this route"));
  }
  try {
